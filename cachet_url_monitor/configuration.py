...
class Configuration(object):
    ...
    def if_trigger_update(self):
        ...
        and only for non-operational ones above the configured thereshold (allowed_fails).
        """

        if self.status != 1:
            self.current_fails = self.current_fails + 1
            self.logger.info('Failure #%s with thershold set to %s' % (self.current_fails, self.allowed_fails))
